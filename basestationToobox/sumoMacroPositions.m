function [ potentialPos, mountedBuilding ] = sumoMacroPositions( outputMap, map )
%SUMOMACROPOSITIONS Find the potential basestation positions for the macrocells
...
% email: ioan.mavromatis@bristol.ac.uk
    ...
    x = outputMap.bbox(1,1)+limit:map.macroDensity*1000:outputMap.bbox(1,2);
    y = outputMap.bbox(2,1)+limit:map.macroDensity*1000:outputMap.bbox(2,2);
    ...
    xMin = abs(outputMap.bbox(1,1) - min(bsPosX));
    yMin = abs(outputMap.bbox(2,1) - min(bsPosY));
    xMax = abs(outputMap.bbox(1,2) - max(bsPosX));
    yMax = abs(outputMap.bbox(2,2) - max(bsPosY));
